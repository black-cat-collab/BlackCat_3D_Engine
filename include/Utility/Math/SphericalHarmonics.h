...
namespace bc
{
	...
	void SHProjectPolarFunction(PolarFunction& polar_function, const SHSample samples[SAMPLE_COUNT], Vector3 result[]) 
	{
		// For each sample
		...
		const float factor = 4.0f * M_PI / SAMPLE_COUNT;
		for (int c = 0; c < SH_COEFFICIENT_COUNT; c++) 
		{
			result[c] *= factor;
		}
	...
	void SHCalulatePhongLobeCoeffs(float result[SH_NUM_BANDS]);
}
