...
#pragma pack ()
    ...
    public:
		...
		float GetAngles(const  Quaternion& q);
		...
		inline Quaternion(float* pData)
		{
			memcpy(&w, pData, sizeof(float) * 4);
		}
		...
		void FromAxis (const Axis& vAxis );
	    Axis ToAxis () const;
        ...
        float SquaredLength() const;  // squared-length
...
#ifdef _MSC_VER
	static_assert(sizeof(Quaternion) == (4 * sizeof(float)), "Error size of Quaternion");
#endif
