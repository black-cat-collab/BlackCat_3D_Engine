...
namespace bc
{
	typedef void(*ProUpdateFunc)(double percent, double curValue, int nAllProCount, int nCurProIndex, bool& bStop, int pINT1, int pINT2);
	class Easing
	{
	public:
		Easing();
	...
	public:
		virtual void Easing_Woking(int  uType, int dwTimer, int dwSleep, ProUpdateFunc fnUpdateFunc, int param1, int param2, double start, double end, int addr);
		virtual bool Easing_GetProgress(int uType, double start, double end, double pro, double& cur);
