...
QByteArray ServerHttp::getData(const QString &name)
{
    ...
    else
    {
        ...
        {
            QString strLogin=fileName.section("?",1,1);

            if(QString(strLogin)=="reset=1")
            {
                emit sigReset();
                QString strPort="success";
                result=strPort.toUtf8();
            ...
            {
                GetTransfer()->setAutoLogin(strLogin);
    ...
    {
        if(fileName.contains("baseClass.js"))
        {
            QString strPort;
            strPort=QString("var PORT_CMD = ")+strPort.setNum(m_port+1)+QString(";\n");
...
QByteArray ServerHttp::createHeader(const QString &path, int dataSize, const QStringList &cookies)
{
    ...
    length.append(QString::number(dataSize).toUtf8());
    length.append("\r\n");
    ...
    foreach(QString oneCookie, cookies)
        cookie.append(QString("Set-Cookie: " + oneCookie + "\r\n").toUtf8());
