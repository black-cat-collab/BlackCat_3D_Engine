...
void InputSimulator::simulateKeyboard(quint16 keyCode, bool state)
{
    if(!m_pWidget)
    {
#if 0//  def Q_OS_UNIX   //arm不支持，需要调整
    ...
    else
    {
        if(state)
        {
           m_pWidget->KeyPress(keyCode);
        }
        else
        {
            m_pWidget->KeyRelease(keyCode);
        }
    }
}
...
void InputSimulator::simulateMouseKeys(quint16 keyCode, bool state)
{
    ...
    if(state)
    {
        ...
        {
        case 0:
            m_pWidget->MousePress(Qt::LeftButton,m_currPosition);
            break;
        case 1:
            m_pWidget->MousePress(Qt::MiddleButton,m_currPosition);
            break;
        case 2:
            m_pWidget->MousePress(Qt::RightButton,m_currPosition);
            break;
        }

        //  qDebug()<<"MousePress:"<<QString::number(m_currPosition.x())<<","<<QString::number(m_currPosition.y());
    ...
    else
    {
        ...
        {
        case 0:
            m_pWidget->MouseRelease(Qt::LeftButton,m_currPosition);
            break;
        case 1:
            m_pWidget->MouseRelease(Qt::MiddleButton,m_currPosition);
            break;
        case 2:
            m_pWidget->MouseRelease(Qt::RightButton,m_currPosition);
            break;
        }
        //  qDebug()<<"MouseRelease:"<<QString::number(m_currPosition.x())<<","<<QString::number(m_currPosition.y());
    }
#else
        if(state)
        {
            ...
            case 0:
                ...
                break;
            case 1:
                m_pWidget->MousePress(Qt::MidButton,m_currPosition);
                break;
            case 2:
                m_pWidget->MousePress(Qt::RightButton,m_currPosition);
                break;
        ...
        else
        {
            ...
            case 0:
                ...
                break;
            case 1:
                m_pWidget->MouseRelease(Qt::MidButton,m_currPosition);
                break;
            case 2:
                m_pWidget->MouseRelease(Qt::RightButton,m_currPosition);
                break;
...
void InputSimulator::simulateMouseMove(quint16 posX, quint16 posY)
{
    ...
    {
        m_currPosition=QPoint(m_screenPosition.x()+posX,m_screenPosition.y()+posY);

        ServerManager::GetInstance()->SetIntPtrValue(m_currPosition.x(),INT_VALUE_CURSOR_X);
        ServerManager::GetInstance()->SetIntPtrValue(m_currPosition.y(),INT_VALUE_CURSOR_Y);



      //   qDebug()<<"simulateMouseMove:"<<QString::number(posX)<<","<<QString::number(posY);
    }
...
void InputSimulator::simulateWheelEvent(bool deltaPos)
{
    ...
    else
    {
         ...
         if(deltaPos)
         {
             m_pWidget->Wheel(m_currPosition,QPoint(-120,-120));
            // m_currPosition=QPoint(m_screenPosition.x()-120,m_screenPosition.y()-120);
         }
         else
         {
             m_pWidget->Wheel(m_currPosition,QPoint(120,120));
...
void InputSimulator::setMouseDelta(qint16 deltaX, qint16 deltaY)
{
    ...
    {
        m_pWidget->Wheel(m_currPosition,QPoint(m_currPosition.x()-deltaX,m_currPosition.y()-deltaY));
        m_currPosition=QPoint(m_screenPosition.x()-deltaX,m_screenPosition.y()-deltaY);
    }
