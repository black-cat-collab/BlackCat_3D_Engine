...
bool NetworkManager::event(QEvent *event)
{
    if (event->type() == NetworkEvent::NetworkProgress)
    {
        ...
        {
            updateProgress(evtProgress->pRequestTask, evtProgress->uiId,
...
void NetworkManager::updateProgress(QMTNetwork::RequestTask* pRequestTask,quint64 uiId, quint64 uiBatchId, qint64 iBytes, qint64 iTotalBytes, bool bDownload)
{
    ...
    if (bDownload)
    {
        emit downloadProgress(pRequestTask,uiId, iBytes, iTotalBytes);
    }
    else
    {
